...
macro "west" : tactic =>
  `(tactic| first | apply step_west; (· simp (config := { decide := true }); done) | fail "cannot step west")
macro "east" : tactic =>
  `(tactic| first | apply step_east; (· simp (config := { decide := true }); done); (· simp (config := { decide := true }); done) | fail "cannot step east")
macro "north" : tactic =>
  `(tactic| first | apply step_north; (· simp (config := { decide := true }); done) | fail "cannot step north")
macro "south" : tactic =>
  `(tactic| first | apply step_south; (· simp (config := { decide := true }); done); (· simp (config := { decide := true }); done) | fail "cannot step south")
